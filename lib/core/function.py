...
def train_adaptive(config, train_III_loader, model_p, model_d, criterion_p, criterion_d, optimizer_p, optimizer_d, epoch, output_dir, tb_log_dir, writer_dict,losses_P_list, losses_D_list, acces_P_list, acces_D_list, acc_num_total, num, losses_p, acc_p, losses_d):
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_III_loader):  # 1004 mixed images to train  
        ...
        ######## StageI: Domain Classifier Update ########
        ...
        ######## StageII: PoseResnet Update ########     
        domain_logits_p = model_d(feature_output)
        
        target = target.cuda(non_blocking=True)
        target_weight = target_weight.cuda(non_blocking=True)
        loss_p = criterion_p(kpt_output, target, target_weight) - 0.0005 * criterion_d(domain_logits_p, domain_label)
        ...
        _, avg_acc, cnt, pred = accuracy(kpt_output.detach().cpu().numpy(),
                                         target.detach().cpu().numpy())
        ...
        if i % config.PRINT_FREQ == 0:
            ...
            writer = writer_dict['writer']
            global_steps = writer_dict['train_global_steps']
            writer.add_scalar('train_loss', losses_p.val, global_steps)
            writer.add_scalar('train_acc', acc_p.val, global_steps)
            writer_dict['train_global_steps'] = global_steps + 1

            prefix = '{}_{}'.format(os.path.join(output_dir, 'train'), i)
            save_debug_images(config, input, meta, target, pred*4, kpt_output,
                              prefix)
...
def validate_adaptive(config, val_loader, val_dataset, model_p, criterion_p, output_dir,
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            # compute output
            _, output = model_p(input)
            if config.TEST.FLIP_TEST:
                # this part is ugly, because pytorch has not supported negative index
                # input_flipped = model(input[:, :, :, ::-1])
                input_flipped = np.flip(input.cpu().numpy(), 3).copy()
                input_flipped = torch.from_numpy(input_flipped).cuda()
                output_flipped = model_p(input_flipped)
...
def validate_feature(config, val_loader, val_dataset, model_p, criterion_p, output_dir,
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            if config.DATASET.TEST_SET == 'validate_infant':
                img_label.append(meta['synthetic'].unsqueeze(-1)*1)
            else:
                img_label.append(2)
            # compute output
            feature_output, output = model_p(input)
            if i == 0:
