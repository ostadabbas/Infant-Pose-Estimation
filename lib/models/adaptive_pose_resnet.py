...
class DomainClassifier(nn.Module):
    ...
    def __init__(self, feature_output):
        ...
        self.planes = 2048 * 12 * 9

        self.layer = nn.Sequential(
            nn.Linear(self.planes, 2048),
            nn.BatchNorm1d(2048),
            nn.ReLU(),
    ...
    def forward(self, feature_output):
        x = feature_output.view(-1, 2048 * 12 * 9)
        out = self.layer(x)
        return out
...
class PoseResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        self.feature_output = self.layer4(x)
        

        # pose network
        a = self.deconv_layers(self.feature_output)
        self.kpt_output = self.final_layer(a)
...
def get_adaptive_PoseResNet(cfg, is_train, **kwargs):
    ...
    model_p = PoseResNet(block_class, layers, cfg, **kwargs)
